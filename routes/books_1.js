...
router.get(/**/, function(req, res){
	Book.findOne({_id: req.params.book_id}, function(err, book){
		...
		if(!book) return res.status(/**/).json({error: 'not found'});
		res.json(book);
	})
});
...
router.get(/**/, function(req, res){
    Book.find({author: req.params.author}, {_id: 0, title: 1, published_date: 1},  function(err, books){
        if(err) return res.status(/**/).json({error: err});
        if(books.length === 0) return res.status(/**/).json({error: 'book not found'});
        res.json(books);
    })
});
...
router.put(/**/, function(req, res){
	...
	Book.findById(req.params.book_id, function(err, book){
        if(err) return res.status(/**/).json({ error: 'database failure' });
        ...
        book.save(function(err){
            if(err) res.status(/**/).json({error: 'failed to update'});
